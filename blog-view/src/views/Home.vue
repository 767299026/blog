  ...
  <div class="home-container">
    ...
    <el-card v-for="blog in blogs" :body-style="{padding: '0px'}" class="home-main-column-middle-card" :key="blog.id">
      <div class="home-title">
        ...
        <div style="font-size: small;color: blue">  创作时间：{{ blog.createTime.split(' ')[0] }}  </div>
        <div style="font-size: small;color: blue">  更新时间：{{ blog.updateTime.split(' ')[0] }}   </div>
        <div style="font-size: small;color: blue">  分类：{{ blog.typeName }}</div>
        <div style="font-size: small;color: blue">  字数：{{ blog.words }}</div>
...
<!--      <div class="home-description-markdown-body" v-html="blog.descriptionMd"  v-viewer="{movable: false}"></div>-->
      ...
      <!--阅读全文按钮-->
      <div class="div-btn">
        <a class="color-btn" href="javascript:;" @click.prevent="toBlog(blog)">阅读本文</a>
      </div>
    </el-card>
